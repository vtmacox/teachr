...
Using the space below create a scatter plot using `ggplot2` with data from the starwars dataframe. Put `height` on the y-axis and `mass` on the x-ais.  HINT: Google "scatter plot ggplot2"
...
Now that you've learned some Advanced R techniques. It's time to learn how data need to be formated in order to analyze it.  Under most conditions, you want your data to be "tidy".  Tidy data is a term made popular by Hadely Wickham (creater of `dplyr` and `ggplot2`).  There are 3 principles of tidy data:
