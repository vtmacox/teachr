...
In the markdown document, create seperate chunks for the following code objectives:
- Read in data
- Assign proper data types to each variable
- Recode the value `9` as `NA` for the `anxiety` variable
- Get the descrptive statistics for each variable (except for the `id` variable)
...
- Create a regression table using `chol` as the dependent variable and `exer`, `fv`, `age`, and `med` as the independent variables
    + HINT: You will need the `lm()` function to do the analysis, the `summary()` to get the results, and the `knitr::kable()` to display the results. 
