...
An object that contains multiple data points of the same type is called a vector.  Our object `numbers` above would be considered a numeric vector and the `char_ob2` object would be considered a character vector. An individual datum in a vector is called an element.  So the first element in our `numbers` object is 1.  We can tell **R** to pull specific element within an object why specifing the elements position in the object. For example:

```{r, echo=TRUE, exercise.setup = "obj"}
numbers[3]
```

The code above pulls the 3rd element out of the object `numbers` which just so happens to be 3.  We do this by referencing the object and then specifiyng the element position in brackets immediatly after referencing the object.
...
In the example above you can see we are assigning the function to the name `add`.  After the `<-` symbol, we use `function()` to say we are creating a function.  Inside the parentheses are the arguments that the user is going to specify. Arguments are seperated by commas.  Everything between the brackets is what our function does.  In this example we take the value `x` supplied by the user and add it to the value `y` which is also supplied by the user. We use the `return()` function to explicitly say we want the value of the object `res` returned (or outputted).  This way we can store the value if we want.
...
In general, you want to create a function when you are useing the same set of code over and over again.  Using a function saves space an time.  For example, if you needed to recode or reformat data, using a function might save you a lot of time.  
...
In this section we will take everything we have learned so far and begin to manipulate data.  We will go over how to create new variables using conditional statements and arithematic fucntions.
...
Let's look at a different database.  We are going to use the `starwars` data contained in the `dplyr` package we loaded before.  This dataframe contains a hight variable and a mass variable.  Let's calculate the BMI for each person and store it as a variable called `bmi` in the starwars database.
...
This function on its own isn't useful since it just is a vector of `TRUE` or `FALSE` values.  But we can use the `is.na()` fucntion in conjunction with the `table()` function to sum the values of true and false.
...
We can also recode variables using conditional statements.  Conditional statements are `if()`, `else()`, and `ifelse()` functions.  We can use these functions to evaluate statements and return values based on wether those statements are `TRUE` or `FALSE`.  Let's take our new `bmi` variable and use the values to classify if they are obese or not and store this into a new variable called `obese`:
...
The first argument of the `ifelse()` function is the conditional statement.  The second arguement is the value to assign the object if the conditional statement is true.  The third arguement is the value to assign the object if the conditional statement is false. 

The code above evaluates wether the `bmi` variable is greater than or equal to 30 (the conditional statement).  If it is true it assigns our new variable `obese` to the value 1.  If it is false, it assigns the variable the value of 0.  We could have assigned it any value we want (like `TRUE` and `FALSE` or "Yes" and "No"), but it's better to use numeric coding when possible.
...
Loops let you use indices to cycle through a sequence of variables.  In this example we will use a `for` loop.  Ther are several types of loops that have pros and cons, but `for` loops are the most common. Here is an example of how to get the means of the three variables from above.
...
```{r color, exercise = T, exercise.eval = TRUE}
...
The easiets way to solve coding problems in **R** is to just Google them.  I would say over 90% of problems I've run in to, I've found solutions through Google.  Since you are just learning though, it might be helpful to have some go to resources other than Google.  Below are a list of resources that I've come across that I've found helpful.  I'll continue to update this list has I find resources.  
